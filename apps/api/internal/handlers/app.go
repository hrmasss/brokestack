...
func (h *AppHandler) Register(app *fiber.App) {
	...
	api.Patch("/provider-accounts/:id/image-settings", h.requireAuth, h.updateProviderAccountImageSettings)
	api.Post("/dev/e2e/workspaces/:id/provider-fixture", h.requireAuth, h.createE2EProviderFixture)
	...
	api.Get("/workspaces/:id/image-jobs", h.requireAuth, h.listWorkspaceImageJobs)
	api.Post("/workspaces/:id/image-jobs", h.requireAuth, h.createWorkspaceImageJob)
	api.Post("/workspaces/:id/image-batches", h.requireAuth, h.createWorkspaceImageBatch)
	api.Get("/image-jobs/:id", h.requireAuth, h.getImageJob)
	api.Get("/image-jobs/:id/outputs", h.requireAuth, h.listImageOutputs)
	api.Get("/image-outputs/:id/content", h.requireAuth, h.getImageOutputContent)
	api.Get("/workspaces/:id/api-keys", h.requireAuth, h.listWorkspaceAPIKeys)
	api.Post("/workspaces/:id/api-keys", h.requireAuth, h.createWorkspaceAPIKey)
	api.Post("/workspace-api-keys/:id/revoke", h.requireAuth, h.revokeWorkspaceAPIKey)
	...
	api.Post("/images", h.publicCreateImageJob)
	api.Post("/image-batches", h.publicCreateImageBatch)
	api.Get("/images/:id", h.publicGetImageJob)
	api.Get("/images/:id/outputs", h.publicListImageOutputs)
